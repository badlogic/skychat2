...
host_dir=/home/badlogic/skychat.marioslab.io
...
if [ "$1" == "server" ]; then
    ...
    ssh -t $host "export SKYCHAT2_DB=$SKYCHAT2_DB && export SKYCHAT2_DB_USER=$SKYCHAT2_DB_USER && export SKYCHAT2_DB_PASSWORD=$SKYCHAT2_DB_PASSWORD && cd $host_dir && ./docker/control.sh stop && ./docker/control.sh start && ./docker/control.sh logs"
else
